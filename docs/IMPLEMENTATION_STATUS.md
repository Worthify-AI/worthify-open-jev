...
- The suite passed 177 tests after the authored long-context trainer and publication updates. Original raw-result hashes and all 69 upstream summary claims still verify.
...
- `results/worthify/experiment-outcomes-20260922.json` preserves the MoE memory proof, interrupted 31B comparison, superseded precision pilot, and transient CUDA startup errors. Incomplete runs have no reported final quality or speed metrics.
...
- Doom/Freedoom and Tetris-style falling-block recording, replay verification, static browser playback, and labelled MP4 export are implemented in `game-demo/`. CPU smoke episodes and eight actual pilot-adapter decisions per game passed verification; these are integration checks, not full-training results. Browser checks cover play/pause, restart, episode switching, frame/decision alignment, and mobile layout. The final recipe records all three predeclared seeds per game with the validation-selected 12B classification adapter after training, evaluation, and private upload verification.
- A follow-on job verifies all six gameplay recordings, publishes their checksummed video bundle to a private Hugging Face dataset, downloads it at an immutable revision, and adds those pinned videos to the classification candidate's model card. Public visibility and the launch article still depend on the completed release evidence.
- The owner supplied a separate RunPod B200 for long-context experiments. `experiments/long_context/` isolates its native FlexAttention masks, kernel checks, checkpoint activation offloading, exact-length systems probes, and authored evidence evaluation from the main trainer. Systems feasibility, frozen-base retrieval checks, and any meaningful long-context fine-tuning are reported separately; none establishes one-million-token support.
- The B200 completed one warmup and two measured forward/backward/optimizer updates at each length from 8,192 through 262,144 tokens. At 262,144, measured updates averaged 580.78 seconds with peak allocated GPU memory of 134.38 GiB. These use repeated synthetic filler and arbitrary labels, so they establish systems feasibility only. Raw receipts, failed kernel attempts, source provenance, and a recomputable summary are in `results/worthify/long-context-systems-20260922/`.
- A separate fixed nine-example authored evidence pilot is queued after frozen-base short/long evaluation. `manifests/authored-long-context-pilot-20260922.json` records its predeclared settings; it uses meaningful labels, disjoint source groups, all 27 validation rows, fresh reload verification, and matched short/long held-out checks. No long-context quality improvement or completed pilot adapter is claimed yet.
...
Early full-data classification rates are about 1.5 examples/second; the evidence pilot measured about 1.46 examples/second. Together they estimate 12–16 hours for both recipes per seed, with the seeds running in parallel, including validation and evaluation. Initial training/evaluation completion is expected by September 23, subject to successful full runs. October 5 remains the public-launch target, conditional on final verification and CI access. No improvement or completed launch is promised.

The subsequent RunPod work has an owner-authorized $70 spending target, with limited extra runway if close to completing training and demos. Its B200 and running storage cost about $6.94/hour. A provider-level stop watchdog protects the rented instance; artifacts must be exported before an early stop, and the persistent volume must be retained. The local A100 jobs continue independently.
