...
def _runtime_identity(root: Path) -> dict:
    ...
    paths = [
        ...
        root / "experiments" / "long_context" / "activation_offload.py",
...
def run_probe(args) -> dict:
    ...
    try:
        ...
        def native_step(index: int) -> dict:
        ...
        def one_step(index: int) -> dict:
            ...
            with context as offload_stats:
                step = native_step(index)
            if args.activation_offload and not offload_stats["offloaded_bytes"]:
                raise RuntimeError("Activation offload copied no checkpoint inputs")
            step["activation_offload"] = offload_stats
            return step
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--activation-offload", action="store_true",
                        help="store checkpoint inputs in CPU RAM after offload equivalence validation")
